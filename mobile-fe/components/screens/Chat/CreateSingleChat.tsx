...
import { View, Text, FlatList, TouchableOpacity, ActivityIndicator, Alert, StyleSheet, Image, TextInput } from 'react-native';
...
const SelectUserScreen = () => {
    ...
    useEffect(() => {
        ...
        const fetchUsers = async () => {
            try {
                ...
                const filteredUsers = response.data.filter(user => user.id !== parseInt(currentUserId));
    ...
    const startChat = async (user2Id, user2Name) => {
        try {
            ...
            const response = await axios.post(`${API_BASE_URL}/chat/start?user2Id=${user2Id}`, null, {
                headers: { Authorization: `Bearer ${token}` },
            });
            ...
            const chatPartnerName = chatNameParts.find(name => name !== myUsername) || response.data.chatName;

            // 📌 Chuyển sang màn hình chat với tên chính xác
            router.push({ pathname: `/chat/${response.data.id}`, params: { chatName: chatPartnerName } });
        ...
        <View style={styles.container}>
            ...
            {loading ? (
                ...
                <FlatList
                    ...
                    renderItem={({ item }) => (
                        <TouchableOpacity style={styles.userCard} onPress={() => startChat(item.id, item.name)}>
...
const styles = StyleSheet.create({
    ...
    avatar: { width: 50, height: 50, borderRadius: 25, marginRight: 10, backgroundColor: '#ddd' },
